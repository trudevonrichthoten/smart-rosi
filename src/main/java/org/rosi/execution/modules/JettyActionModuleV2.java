...
public class JettyActionModuleV2 extends RosiModule {
   ...
   public JettyActionModuleV2( String moduleName , ModuleContext context  )
      throws Exception {
      ...
      log( "Context created with the following settings (V2) ");
      log( "              port " +_port);
   ...
   public void start() throws Exception {
        ...
        ServletHolder holder2 = new ServletHolder(
            new MemoryServlet( JettyActionModuleV2.this , _memory )
        ) ;
   ...
   public void runRosiToNetwork(){
       ...
       while( ! Thread.interrupted() ){
          ...
          try{
             ...
             RosiSetterCommand command = (RosiSetterCommand) c ; 

             String key = command.getKey() ;

             if( _inFilter != null ){

                 String [] sub = _inFilter.translate( command.getKey() ) ;

                 if( ( sub != null ) && ( sub.length >=1 ) )key = sub[0] ;
             }
  
             log("Setter '"+command.getSource()+"' -> '"+getName()+"' cmd="+command+" ("+key+")" ) ;
             _memory.add( key , command.getValue() ) ;
